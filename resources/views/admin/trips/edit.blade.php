                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                         ...
                         <div>
                              ...
                              <p class="hero-subtitle">
                                   Perbarui data perjalanan, jadwal, rute, kendaraan, driver, waktu, status, dan catatan
                                   operasional.
                              </p>
                         </div>
               ...
               <div class="form-card">
                    ...
                    <div class="form-card-header">
                         ...
                         <p class="form-card-subtitle">
                              Trip code dibuat otomatis dan tidak dapat diubah.
                         </p>
                    </div>
                         ...
                         <div class="row g-3">
                              ...
                              <div class="col-lg-4 col-md-6">
                                   ...
                                   <div class="input-icon">
                                        ...
                                        <input type="text" value="{{ $trip->trip_code }}"
                                             class="form-control readonly-box" readonly>
                                   </div>
                              </div>
                              ...
                              <div class="col-lg-4 col-md-6">
                                             ...
                                             @foreach ($schedules as $schedule)
                                                  @php
                                                       $scheduleName =
                                                           ...
                                                           (data_get($schedule, 'schedule_name') ??
                                                               (data_get($schedule, 'schedule_code') ??
                                                                   (data_get($schedule, 'code') ??
                                                                       'Schedule ID: ' . $schedule->id)));
                                                  @endphp

                                                  <option value="{{ $schedule->id }}"
                                                       {{ old('schedule_id', $trip->schedule_id) == $schedule->id ? 'selected' : '' }}>
                                                       {{ $scheduleName }}
                                                  </option>
                                             @endforeach
                                   ...
                                   <div class="help-text">
                                        Boleh dikosongkan jika trip tidak memakai schedule.
                                   </div>
                              </div>
                                   ...
                                   <div class="input-icon">
                                        ...
                                        <select name="status" class="form-select @error('status') is-invalid @enderror">
                                             ...
                                             <option value="scheduled"
                                                  {{ old('status', $trip->status) == 'scheduled' ? 'selected' : '' }}>
                                                  Scheduled
                                             </option>

                                             <option value="running"
                                                  {{ old('status', $trip->status) == 'running' ? 'selected' : '' }}>
                                                  Running
                                             </option>

                                             <option value="completed"
                                                  {{ old('status', $trip->status) == 'completed' ? 'selected' : '' }}>
                                                  Completed
                                             </option>

                                             <option value="cancelled"
                                                  {{ old('status', $trip->status) == 'cancelled' ? 'selected' : '' }}>
                                                  Cancelled
                                             </option>

                                             <option value="delayed"
                                                  {{ old('status', $trip->status) == 'delayed' ? 'selected' : '' }}>
                                                  Delayed
                                             </option>
                                        ...
                                        <select name="route_id" class="form-select @error('route_id') is-invalid @enderror">
                                             ...
                                             @foreach ($routes as $route)
                                                  @php
                                                       ...
                                                       $routeStart =
                                                           data_get($route, 'start_location') ??
                                                           (data_get($route, 'origin') ??
                                                               (data_get($route, 'from_location') ?? null));

                                                       $routeEnd =
                                                           data_get($route, 'end_location') ??
                                                           (data_get($route, 'destination') ??
                                                               (data_get($route, 'to_location') ?? null));
                                                  @endphp
                                   ...
                                   <div class="input-icon">
                                        ...
                                        <select name="vehicle_id"
                                             ...
                                             @foreach ($vehicles as $vehicle)
                                                  @php
                                                       $vehicleName =
                                                           data_get($vehicle, 'vehicle_number') ??
                                                           (data_get($vehicle, 'plate_number') ??
                                                               (data_get($vehicle, 'license_plate') ??
                                                                   (data_get($vehicle, 'police_number') ??
                                                                       (data_get($vehicle, 'name') ??
                                                                           'Vehicle ID: ' . $vehicle->id))));

                                                       $vehicleType =
                                                           data_get($vehicle, 'type') ??
                                                           (data_get($vehicle, 'vehicle_type') ??
                                                               (data_get($vehicle, 'brand') ?? null));
                                                  @endphp
                                             ...
                                             @foreach ($drivers as $driver)
                                                  @php
                                                       $driverName =
                                                           ...
                                                           (data_get($driver, 'driver_name') ??
                                                               (data_get($driver, 'full_name') ??
                                                                   'Driver ID: ' . $driver->id));

                                                       $driverPhone =
                                                           data_get($driver, 'phone') ??
                                                           (data_get($driver, 'phone_number') ??
                                                               (data_get($driver, 'no_hp') ?? null));
                                                  @endphp
                              ...
                              <div class="col-lg-3 col-md-6">
                                   ...
                                   <div class="input-icon">
                                        ...
                                        <input type="datetime-local" name="planned_start_time"
                                             value="{{ old('planned_start_time', $formatForInput($trip->planned_start_time)) }}"
                              ...
                              <div class="col-lg-3 col-md-6">
                                   ...
                                   <div class="input-icon">
                                        ...
                                        <input type="datetime-local" name="planned_end_time"
                                             value="{{ old('planned_end_time', $formatForInput($trip->planned_end_time)) }}"
                              ...
                              <div class="col-lg-3 col-md-6">
                                   ...
                                   <div class="input-icon">
                                        ...
                                        <input type="datetime-local" name="actual_start_time"
                                             value="{{ old('actual_start_time', $formatForInput($trip->actual_start_time)) }}"
                              ...
                              <div class="col-lg-3 col-md-6">
                                   ...
                                   <div class="input-icon">
                                        ...
                                        <input type="datetime-local" name="actual_end_time"
                                             value="{{ old('actual_end_time', $formatForInput($trip->actual_end_time)) }}"
                         ...
                         <div class="row g-3">
                              ...
                              <div class="col-lg-4 col-md-6">
                                   ...
                                   <div class="input-icon">
                                        ...
                                        <input type="number" name="delay_minutes"
                                             value="{{ old('delay_minutes', $trip->delay_minutes ?? 0) }}" min="0"
                              ...
                              <div class="col-lg-8 col-md-6">
                                   ...
                                   <textarea name="notes" rows="4" class="form-control @error('notes') is-invalid @enderror"
                                        placeholder="Masukkan catatan perjalanan jika ada">{{ old('notes', $trip->notes) }}</textarea>
